...
#   statistics for the MHC1_sa and MHC0 groups.
#
...
def chief_complaint_group_enriched_terms_query(
    enriched_group: str,
    limit_per_group: int = 20,
    min_admissions_in_either_group: int = 10,
) -> str:
    if enriched_group not in {"MHC1_sa", "MHC0"}:
        raise ValueError("enriched_group must be either 'MHC1_sa' or 'MHC0'")
    ...
    paired AS (
        SELECT
            mhc1.word,
            mhc1.n_admissions_with_word AS n_mhc1,
            mhc1.pct_admissions_with_word AS pct_mhc1,
            mhc0.n_admissions_with_word AS n_mhc0,
            mhc0.pct_admissions_with_word AS pct_mhc0,
            mhc1.pct_admissions_with_word / mhc0.pct_admissions_with_word AS ratio_mhc1_over_mhc0,
            mhc0.pct_admissions_with_word / mhc1.pct_admissions_with_word AS ratio_mhc0_over_mhc1
        FROM metrics AS mhc1
        JOIN metrics AS mhc0 ON mhc1.word = mhc0.word
        WHERE mhc1.source_table = 'MHC1_sa'
          AND mhc0.source_table = 'MHC0'
          AND greatest(mhc1.n_admissions_with_word, mhc0.n_admissions_with_word)
    ...
    ranked AS (
        SELECT
            ...
            n_mhc0,
            pct_mhc0,
            ratio_mhc1_over_mhc0,
            ratio_mhc0_over_mhc1,
            CASE
                WHEN ratio_mhc1_over_mhc0 >= ratio_mhc0_over_mhc1 THEN 'MHC1_sa'
                ELSE 'MHC0'
            END AS enriched_in,
            CASE
                WHEN ratio_mhc1_over_mhc0 >= ratio_mhc0_over_mhc1 THEN ratio_mhc1_over_mhc0
                ELSE ratio_mhc0_over_mhc1
...
def main() -> None:
    queries = [
        ...
        (
            "Chief complaint terms enriched in MHC1_sa",
            chief_complaint_group_enriched_terms_query("MHC1_sa"),
            "chief_complaint_terms_enriched_in_MHC1_sa.csv",
        ),
        (
            "Chief complaint terms enriched in MHC0",
            chief_complaint_group_enriched_terms_query("MHC0"),
            "chief_complaint_terms_enriched_in_MHC0.csv",
        ),
    ]
