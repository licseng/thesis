...
def main() -> None:
    ...
    admissions_per_subject_summary.to_csv(
        common.OUTPUT_DIR / "matched_cohort_admissions_per_subject_summary.csv",
        index=False,
    )
    subject_categorical_distribution.to_csv(
        common.OUTPUT_DIR / "matched_cohort_subject_categorical_distribution.csv",
        index=False,
    )
    subject_categorical_balance.to_csv(
        common.OUTPUT_DIR / "matched_cohort_subject_categorical_balance.csv",
        index=False,
    )
    subject_utilization_counts.to_csv(
        common.OUTPUT_DIR / "matched_cohort_subject_utilization_counts.csv",
        index=False,
    )
    subject_utilization_summary.to_csv(
        common.OUTPUT_DIR / "matched_cohort_subject_utilization_summary.csv",
        index=False,
    )

    print(f"Saved subject-level characterization outputs to: {common.OUTPUT_DIR}")
